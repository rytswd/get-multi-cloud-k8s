...
resource "google_compute_router_interface" "gcp_to_aws_1" {
  name       = "interface-to-aws-ha-1"
  router     = google_compute_router.main.name
  ip_range   = "${aws_vpn_connection.cx_1.tunnel1_cgw_inside_address}/30"
  vpn_tunnel = google_compute_vpn_tunnel.gcp_to_aws_1.name
}

resource "google_compute_router_interface" "gcp_to_aws_2" {
  name       = "interface-to-aws-ha-2"
  router     = google_compute_router.main.name
  ip_range   = "${aws_vpn_connection.cx_1.tunnel2_cgw_inside_address}/30"
  vpn_tunnel = google_compute_vpn_tunnel.gcp_to_aws_2.name
}

resource "google_compute_router_interface" "gcp_to_aws_3" {
  name       = "interface-to-aws-ha-3"
  router     = google_compute_router.main.name
  ip_range   = "${aws_vpn_connection.cx_2.tunnel1_cgw_inside_address}/30"
  vpn_tunnel = google_compute_vpn_tunnel.gcp_to_aws_3.name
}

resource "google_compute_router_interface" "gcp_to_aws_4" {
  name       = "interface-to-aws-ha-4"
  router     = google_compute_router.main.name
  ip_range   = "${aws_vpn_connection.cx_2.tunnel2_cgw_inside_address}/30"
  vpn_tunnel = google_compute_vpn_tunnel.gcp_to_aws_4.name
}

resource "google_compute_router_peer" "gcp_to_aws_1" {
  name                      = "peer-aws-1"
  router                    = google_compute_router.main.name
  peer_ip_address           = aws_vpn_connection.cx_1.tunnel1_vgw_inside_address
  peer_asn                  = aws_vpn_connection.cx_1.tunnel1_bgp_asn
  advertised_route_priority = 100
  interface                 = google_compute_router_interface.gcp_to_aws_1.name
}

resource "google_compute_router_peer" "gcp_to_aws_2" {
  name                      = "peer-aws-2"
  router                    = google_compute_router.main.name
  peer_ip_address           = aws_vpn_connection.cx_1.tunnel2_vgw_inside_address
  peer_asn                  = aws_vpn_connection.cx_1.tunnel2_bgp_asn
  advertised_route_priority = 100
  interface                 = google_compute_router_interface.gcp_to_aws_2.name
}

resource "google_compute_router_peer" "gcp_to_aws_3" {
  name                      = "peer-aws-3"
  router                    = google_compute_router.main.name
  peer_ip_address           = aws_vpn_connection.cx_2.tunnel1_vgw_inside_address
  peer_asn                  = aws_vpn_connection.cx_2.tunnel1_bgp_asn
  advertised_route_priority = 100
  interface                 = google_compute_router_interface.gcp_to_aws_3.name
}

resource "google_compute_router_peer" "gcp_to_aws_4" {
  name                      = "peer-aws-4"
  router                    = google_compute_router.main.name
